...
const Categories = ({ category, setCategory, setSearchQuery }) => {
  ...
  const filteredCategories = auctionData
    ? auctionData.map((item) =>
        item.category.map(
          (i) => (allCategories.includes(i) ? null : allCategories.push(i)) // checks if category of item exists in allCategories, if not - category will be appended, if yes - cat will be skipped
        )
      )
    : [];
    ...
    <>
      <div className="container d-flex flex-row justify-content-between border border-secondary rounded p-2 ">
        ...
        <Button
          ...
          onClick={() => {
            setToggle(!toggle), filteredCategories;
          }}
        >
          ...
          <div className="list-group">
            ...
            {allCategories.map((cat, index) => (
              <Link
                ...
                to={`/SearchPage/${cat}`}
                onClick={() => {
                  setCategory(cat), setToggle(false);
                }}
              >
                {cat}
              </Link>
